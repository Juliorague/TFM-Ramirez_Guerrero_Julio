...
homo_vs_absent <- metab_1 %>%
  filter(graft_type %in% c("homo", "absent"))

# transform the dataset from long to wide format
homo_vs_absent <- homo_vs_absent %>%
  pivot_wider(names_from = identifier, values_from = expression)

## data splitting
metab_dt <- select(homo_vs_absent, -c(individual))
...
preprocessing_recipe <-
  recipes::recipe(graft_type ~ ., data = metab_train) %>%
...
metab_cv <- vfold_cv(metab_train, v=5, repeats = 5, strata = graft_type)
