...
# metabolites
mm_metab <- metab_1 %>%
  gather(key = "identifier", value = "expression", -c(individual, sample_type_origin, rootstock_type))
...
metab_rec <- recipe(rootstock_type ~ ., data = metab_train) %>%
...
mm_train <- metab_train %>%                      
  gather(key = "identifier", value = "expression", -c(rootstock_type))
...
metab_cv <- vfold_cv(metab_train, v=5, repeats = 10, strata = rootstock_type)
...
lr_auc <- lr_res %>%
  collect_predictions() %>%
  roc_curve(rootstock_type, .pred_sensitive) %>%
