...
# metabolites
mm_metab <- metab_1 %>%
  gather(key = "identifier", value = "expression", -c(individual, sample_type_origin, rootstock_graft))
...
metab_rec <- recipe(rootstock_graft ~ ., data = metab_train) %>%
...
mm_train <- metab_train %>%                      
  gather(key = "identifier", value = "expression", -c(rootstock_graft))
...
metab_cv <- vfold_cv(metab_train, v=5, repeats = 10, strata = rootstock_graft)
...
lr_auc <- lr_res %>%
  collect_predictions() %>%
  roc_curve(rootstock_graft, 
            .pred_sensitive_absent,   
            .pred_sensitive_Galicia,                       
            .pred_sensitive_homo, 
            .pred_tolerant_absent,
            .pred_tolerant_Galicia,
            .pred_tolerant_homo) %>%
